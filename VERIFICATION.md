...
- CI validates matrix references against real test functions via `scripts/validate_assumption_matrix.py`.
- CI additionally publishes a unified `review-package` artifact generated by `scripts/review_package.sh` (logs + SARIF + LCOV + checksums + manifest).
