...
| Non-standard token composability (rebasing/FoT/inflationary/output-semantic variants) | Explicitly outside supported proof envelope | Rejected where enforced; otherwise explicitly flagged unsupported via adversarial tests | See token compatibility docs |
...
Process references:
- [`PROTOCOL_TEMPLATE.md`](PROTOCOL_TEMPLATE.md)
- [`PROTOCOL_HANDOFF_CHECKLIST.md`](PROTOCOL_HANDOFF_CHECKLIST.md)
- [`Protocol/README.md`](Protocol/README.md)

Rehearse strict-gate intake against live RigidityCore target models (Aave/Balancer session examples):
...
- Public boundary and handling policy: [`strategy/README.md`](strategy/README.md)
- Public evidence-portal description: [`strategy/EVIDENCE_PORTAL.md`](strategy/EVIDENCE_PORTAL.md)

Sanitized templates and schema references are in:
...
Set `ENABLE_COMMERCIAL_AUTOMATION=true` (repository variable) only in private/forked environments where commercial workflows should run.
...
- Workflow execution is gated behind repo variable `ENABLE_COMMERCIAL_AUTOMATION=true`.
...
- Protocol intake gate report (`protocol-intake-artifacts`) generated from template payloads
- Unified review bundle (`review-package` artifact) generated by `scripts/review_package.sh`
  (includes theorem inventory, Lean/Forge/coverage logs, Slither SARIF, strict intake report, and checksums)
- Commercial artifacts (`commercial-review-package`, `evidence-portal`, `case-study-publish`, `kpi-outbound-digest`) are available only when `ENABLE_COMMERCIAL_AUTOMATION=true`.
